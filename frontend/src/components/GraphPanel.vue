...
const renderGraph = () => {
  ...
  const getLinkPath = (d) => {
    ...
    // Perpendicular offset from the line direction, calculated proportional to distance for visible curves
  ...
  // Keep references so the label toggle watcher can update visibility
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      .on('start', (event, d) => {
        // Track the initial position without restarting the simulation yet
        d.fx = d.x
        d.fy = d.y
      ...
      .on('drag', (event, d) => {
        // Only treat this as a drag after movement passes a small threshold
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)
        
        if (!d._isDragging && distance > 3) {
          // Restart the simulation only when a real drag begins
          d._isDragging = true
      ...
      .on('end', (event, d) => {
        // Only cool the simulation if the node was actually dragged
        if (d._isDragging) {
  ...
  simulation.on('tick', () => {
    ...
    // Update edge label positions while keeping text horizontal
    linkLabels.each(function(d) {
      const mid = getLinkMidpoint(d)
      d3.select(this)
        .attr('x', mid.x)
        .attr('y', mid.y)
        .attr('transform', '') // Keep labels horizontal
    })
    ...
    linkLabelBg.each(function(d, i) {
      ...
      d3.select(this)
        ...
        .attr('transform', '') // Keep label backgrounds horizontal
    })
  ...
  // Click on empty space to clear the selection panel
  svg.on('click', () => {
    selectedItem.value = null
    node.attr('stroke', '#fff').attr('stroke-width', 2.5)
    linkGroup.selectAll('path').attr('stroke', '#C0C0C0').attr('stroke-width', 1.5)
...
// React to the edge-label visibility toggle
...
/* Hint styling after graph rendering finishes */
