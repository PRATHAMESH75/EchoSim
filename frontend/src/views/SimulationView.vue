  ...
  <div class="main-view">
    ...
    <header class="app-header">
      <div class="header-left">
        <div class="brand" @click="router.push('/')">SWARM</div>
      </div>
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      try {
        ...
        addLog(`Environment shutdown error: ${closeErr.message}`)
        // Graceful shutdown exception, try force stop
        await forceStopSimulation()
      }
    } else {
      ...
      if (simRes.success && simRes.data?.status === 'running') {
        addLog('Detected running simulation, stopping...')
        await forceStopSimulation()
      }
    }
  } catch (err) {
    // Environment status check failure doesn't block subsequent flow
