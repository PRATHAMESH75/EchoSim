...
def create_simulation():
    ...
    try:
        ...
        if not graph_id:
            return jsonify({
                "success": False,
                "error": "Project does not have a graph yet, please call /api/graph/build first"
            }), 400
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    """
    Check whether a simulation has been prepared
    ...
    # List of required files (scripts are not included; they are in backend/scripts/)
    required_files = [
        "state.json",
    ...
    try:
        ...
        # - completed: run complete, so preparation was completed earlier
...
def prepare_simulation():
    ...
    4. Intelligently generate simulation config using LLM (with retry mechanism)
    ...
    try:
        ...
        # ========== Synchronously get entity count (before starting the background task) ==========
...
def get_simulation_history():
    """
    Get history of simulations (with project details)

    Used for the homepage history display; returns a rich list of simulations
    ...
    try:
        ...
        for sim in simulations:
            ...
            # Get run state (read actual rounds from run_state.json as set by the user)
...
def get_simulation_profiles_realtime(simulation_id: str):
    ...
    try:
        ...
        # Check whether generation is in progress (determined from state.json)
        is_generating = False
        total_expected = None
...
def get_simulation_config_realtime(simulation_id: str):
    ...
    - Suitable for real-time viewing during generation
    - Returns extra metadata (e.g. file modification time, whether generation is in progress)
    - Returns partial information even if config is not yet complete
...
def get_simulation_config(simulation_id: str):
    """
    Get simulation config (complete LLM-generated config)
...
def start_simulation():
    ...
    Request (JSON):
        {
            ...
            "max_rounds": 100,                     // optional: maximum simulation rounds
    ...
    About the force parameter:
        - When enabled, stops and clears run logs if simulation is running or complete
                ...
                if state.status == SimulationStatus.RUNNING:
                    ...
                    if run_state and run_state.runner_status.value == "running":
                        ...
                        if force:
                            ...
                            return jsonify({
                                "success": False,
                                "error": "Simulation is running; please call /stop first, or use force=true to force a restart"
                            }), 400
...
def get_simulation_timeline(simulation_id: str):
    ...
    Used by the frontend to display a progress bar and timeline view
...
def interview_agent():
    ...
    (simulation loop completed and in waiting-for-command mode)
    ...
    Response (no platform specified, both-platform mode):
        {
            "success": true,
            "data": {
                "agent_id": 0,
...
def interview_agents_batch():
    ...
    Request (JSON):
        {
            ...
            "platform": "reddit",              // optional, default platform (overridden per item)
                                               // if not specified: both platforms for each Agent simultaneously
    ...
    try:
        ...
        # Optimize each interview item's prompt to prevent Agent from calling tools
    ...
    Request (JSON):
        {
            ...
            "agent_id": 0,                 // optional, get history only for this Agent
