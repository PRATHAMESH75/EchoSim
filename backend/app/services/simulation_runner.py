...
Runs simulations in the background and logs each Agent's actions, supporting real-time status monitoring
"""
...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        # Compute total rounds from time config
        time_config = config.get("time_config", {})
        total_hours = time_config.get("total_simulation_hours", 72)
        minutes_per_round = time_config.get("minutes_per_round", 30)
        total_rounds = int(total_hours * 60 / minutes_per_round)

        # Apply caller-supplied max_rounds, then enforce hard cap
        if max_rounds is not None and max_rounds > 0:
            total_rounds = min(total_rounds, max_rounds)
        if total_rounds > MAX_ROUNDS_CAP:
            logger.warning(
                "total_rounds %d exceeds hard cap %d — truncating", total_rounds, MAX_ROUNDS_CAP
            )
            total_rounds = MAX_ROUNDS_CAP
        ...
        try:
            ...
            # Set working directory to simulation directory (databases and other files will be created there)
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        ...
        Determines whether a platform is enabled by checking if the corresponding
    ...
    def _terminate_process(cls, process: subprocess.Popen, simulation_id: str, timeout: int = 10):
        ...
        if IS_WINDOWS:
            ...
            try:
                ...
                # If still not done after timeout, force send SIGKILL
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # Get platform: prefer the platform in the record, otherwise use the default platform
