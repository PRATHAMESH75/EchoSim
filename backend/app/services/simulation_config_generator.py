...
Achieves full automation without the need for manual parameter configuration.

Adopts a step-by-step generation strategy to avoid failures caused by generating excessively long content all at once:
...
class AgentActivityConfig:
    ...
    # Influence weight (determines the probability that its posts are seen by other Agents)
...
class PlatformConfig:
    ...
    # Viral spread threshold (number of interactions to trigger diffusion)
...
- The user base is Chinese, so the configuration must conform to Beijing time daily schedule habits
- Almost no activity from midnight to 5am (activity multiplier 0.05)
- Gradually active from 6am to 8am (activity multiplier 0.4)
- Moderate activity during working hours 9am-6pm (activity multiplier 0.7)
- Evening 7pm-10pm is the peak period (activity multiplier 1.5)
- Activity declines after 11pm (activity multiplier 0.5)
- General pattern: low activity in early morning, increasing in the morning, moderate during working hours, peak in the evening
- **Important**: The example values below are for reference only; you need to adjust specific time slots based on the nature of the event and the characteristics of the participating group
  - For example: the peak for student groups may be 9pm-11pm; media outlets are active all day; official institutions only during working hours
  - For example: a breaking trending topic may cause discussion even late at night, so off_peak_hours can be appropriately shortened
...
- peak_hours (int array): Peak hours; adjust based on the event's participating group
...
{{
    ...
    def _assign_initial_post_agents(
        ...
        # Type alias mapping (to handle different formats the LLM might output)
        type_aliases = {
...
- **Time must conform to Chinese daily schedule**: almost no activity from midnight to 5am, most active from 7pm to 10pm
...
{{
    ...
    def _generate_agent_config_by_rule(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        if entity_type in ["university", "governmentagency", "ngo"]:
            ...
            # Experts/Professors: active during working hours and evening, medium frequency
            return {
                "activity_level": 0.4,
                "posts_per_hour": 0.3,
